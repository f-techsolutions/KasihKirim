...
--
-- This file lives in supabase/testing/ -- a SIBLING of supabase/tests/, not a
-- subdirectory of it. pg_prove searches the tests directory RECURSIVELY, so
-- supabase/tests/helpers/ was still discovered and reported as a file with
-- "no plan"; only moving outside the tests tree entirely excludes it.
--
-- Install it once, after `supabase db reset` and before `supabase test db`:
--     psql "$DB_URL" -f supabase/testing/00_helpers.sql
